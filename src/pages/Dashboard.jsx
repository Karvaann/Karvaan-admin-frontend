...
const Dashboard = () => {
    ...
    useEffect(() => {
        const fetchSummaryData = async () => {
            const response = await axios.get('http://localhost:8080/logs/get-user-logs/689000000000000000000003', { headers } );
        ...
        <div className="transition-all duration-500 ease-in-out flex-1 ml-[44px] scale-100 w-[95vw] pl-[3%] mt-10">
            {!calenderShow && <div className="flex flex-col items-start self-stretch rounded-[16px] border border-gray-100 bg-white shadow-sm w-[100%]">
                <div className="flex justify-between items-center w-[100%] px-5 py-4 border-b border-[#E2E1E1]">
                    {summaryData?.currentUserPendingTaskCount === 0 && <div className="flex items-center h-8 px-4 py-2 rounded-full bg-[#DCFCE7] text-[#166534] font-poppins text-base font-semibold leading-8">Hurrah! You are up to Date</div>}
                    {summaryData?.currentUserPendingTaskCount !== 0 && <div className="flex items-center h-8 px-4 py-2 rounded-full bg-[#fcdcdc] text-[#651616] font-poppins text-base font-semibold leading-8">Pending Tasks: {summaryData?.currentUserPendingTaskCount}</div>}
                ...
                <div className="flex w-[100%] px-5 py-4">
                    <div className="flex justify-between items-center gap-4 w-full flex-wrap">
                        {
                            summaryData.dateWiseLogs && Object.keys(summaryData.dateWiseLogs).map((date, index) => {
            ...
            {!calenderShow && <div className="flex gap-6 mt-6">
                <div className="bg-white p-6 rounded-xl shadow w-1/3">
                    ...
                    <li className="flex justify-between">
                        <span>Completed</span>
                        <span className="text-green-500 font-semibold">{summaryData?.percentageLogs?.completedCount}</span>
                    </li>
                    <li className="flex justify-between">
                        <span>In Progress</span>
                        <span className="text-blue-500 font-semibold">{summaryData?.percentageLogs?.inProgressCount}</span>
                    </li>
                    <li className="flex justify-between">
                        <span>Pending</span>
                        <span className="text-orange-500 font-semibold">{summaryData?.percentageLogs?.pendingCount}</span>
                    </li>
                    </ul>
                    <div className="w-full h-2 bg-gray-200 rounded-full mb-2">
                    <div className="h-full bg-teal-900 rounded-full" style={{ width: summaryData?.percentageLogs?.completedPercent }}></div>
                    </div>
                    <p className="text-sm text-gray-500 text-center">{summaryData?.percentageLogs?.completedPercent} completion rate</p>
                </div>
                ...
                <div className="bg-white p-6 rounded-xl shadow w-1/3">
                    ...
                    {summaryData.teamPercentCompleteLogs && Object.keys(summaryData.teamPercentCompleteLogs).map((member, idx) => {
                        const color = getRandomDarkBgClass();
                    return (
                        <div key={idx} className="mb-4">
                            <div className="flex items-center gap-2 mb-1">
                            <div className={`w-6 h-6 text-white font-medium flex items-center justify-center rounded-full ${color}`}>
                                {member[0]}
                            </div>
                            <span className="text-gray-700 font-medium">{member}</span>
                            <span className="ml-auto text-gray-700 font-medium">{summaryData.teamPercentCompleteLogs[member]}%</span>
                            </div>
                            <div className="w-full h-2 bg-gray-200 rounded-full">
                            <div className={`h-full rounded-full ${color}`} style={{ width: `${summaryData.teamPercentCompleteLogs[member]}%` }}></div>
                            </div>
                        </div>
                    )
                }
                ...
                <div className="bg-white p-6 rounded-xl shadow w-1/3">
                    ...
                    {summaryData.recentLogs && summaryData.recentLogs.map((item, idx) => {
                        const color = getRandomDarkBgClass();
