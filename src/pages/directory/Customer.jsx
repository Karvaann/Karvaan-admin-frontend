  ...
  const fetchDataBySearch = async () => {
    try {
      if (searchTerm.trim() === "") {
        ...
        const response = await axios.get(
          `http://localhost:8080/customer/get-customer/${searchTerm}`,
          { headers }
        );
        setData(response.data);
      }
    ...
    if (data?.customers && data.customers.length > 0) {
        ...
        const temp = (
          <>
            ...
            <td className="px-4 py-3 flex items-center gap-2">
              <div
                className={`font-bold text-white rounded-full ${getRandomDarkBgClass()} w-[30px] h-[30px] flex items-center justify-center`}
              >
                {data.customers[i].name[0]}
              </div>
              <div>{data.customers[i].name}</div>
            </td>
            <td className="px-4 py-3">
              <div
                className="font-bold text-amber-300 text-[10px] rounded-full bg-amber-100 pt-1 pb-1 pl-3 pr-3"
                style={{ width: "max-content" }}
              >
                {data.customers[i].tier}
              </div>
            </td>
            <td className="px-4 py-3">{data.customers[i].ownerId.name}</td>
            <td className="px-4 py-3">
              <button
                onClick={() => handleRowClick(data.customers[i])}
      ...
      const temp = (
        <>
          ...
          <td className="px-4 py-3">
            <button
              onClick={() => handleRowClick(data.customer)}
              className="bg-gray-200 text-white px-4 py-2 rounded"
            >
              <img src="/assets/icons/eye.svg" alt="" />
            </button>
