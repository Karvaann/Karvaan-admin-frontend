...
const Teams = () => {
    ...
    const fetchDataBySearch = async () => {
            try {
                if (searchTerm.trim() === "") {
                    ...
                    const response = await axios.get(`http://localhost:8080/team/get-team/${searchTerm}`, { headers });
                    setData(response.data);
                }
    ...
    const columns = ["Agent ID", "Name", "Email", "Role", "Notes", "Action"];
        ...
        if (data && data.length > 0) {
            for (let i = 0; i < data.length; i++) {
                ...
                <td className="px-4 py-3 flex items-center gap-2">
                    <div className="font-bold text-white rounded-full bg-amber-700 w-[30px] h-[30px] flex items-center justify-center">{data[i].name[0]}</div>
                    <div>{data[i].name}</div>
                </td>
                ...
                <td className="px-4 py-3"><button onClick={() => handleRowClick(data[i])} className="bg-gray-200 text-white px-4 py-2 rounded"><img src="/assets/icons/eye.svg" alt="" /></button></td>
                <td className="px-4 py-3">
            ...
            const temp = <>
                ...
                <td className="px-4 py-3 flex items-center gap-2">
                    <div className="font-bold text-white rounded-full bg-amber-700 w-[30px] h-[30px] flex items-center justify-center">{data.team.name[0]}</div>
                    <div>{data.team.name}</div>
                </td>
                <td className="px-4 py-3">{data.team.email}</td>
                <td className="px-4 py-3">{data.team.roleId.roleName}</td>
                <td className="px-4 py-3"><button onClick={() => handleRowClick(data.team)} className="bg-gray-200 text-white px-4 py-2 rounded"><img src="/assets/icons/eye.svg" alt="" /></button></td>
                <td className="px-4 py-3">
                    <button className="bg-gray-200 text-white px-4 py-2 rounded"><img src="/assets/icons/trash.svg" alt="" /></button>
                </td>
                </>
